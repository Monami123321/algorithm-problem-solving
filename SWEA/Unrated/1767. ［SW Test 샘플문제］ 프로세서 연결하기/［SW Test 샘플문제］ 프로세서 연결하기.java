...
public class Solution {
    ...
    static boolean check(int r, int c, int dir) {
        for (int i = 1;; i++) {
            if(r+dr[dir]*i<0 || r+dr[dir]*i>n-1 || c+dc[dir]*i<0 || c+dc[dir]*i>n-1) return true;
...
static void connect(int r, int c, int dir) {
    for (int i = 1;; i++) {
        if(r+dr[dir]*i<0 || r+dr[dir]*i>n-1 || c+dc[dir]*i<0 || c+dc[dir]*i>n-1) return;
...
static void dfs(int r, int c, int depth) {
    if(connected+core-depth<connectedAns) return;
    if(depth==core) {
    	if (connected < connectedAns) return;
        int sum = 0;
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                if(map[i][j] ==-1) sum++;
